...
- Grok/codeproxy 的密钥、模型和 Messages 兼容请求地址：用于自动发现公开讨论与竞品洞察；
- AI Responses 兼容的密钥、模型和请求地址：用于定位建议、语义评分、草稿和增长记忆；
- 一个可选的公开 X 主页地址：用于生成可编辑的定位初稿。

设置和新工作台数据会保存到本机 SQLite 数据库，因此同一台电脑、同一端口下打开的不同浏览器会读取同一份数据。Grok 请求地址默认为 `https://codeproxy.dev/v1/messages`，AI 请求地址默认为 `https://codeproxy.dev/v1/responses`，可在设置页改为其他兼容接口。首次升级只会迁移当前浏览器中已有的 Grok、AI 和 X 主页设置；旧定位、队列、评分、草稿、反馈和增长记忆不会迁移，方便从空白工作台重新测试。
